...
typedef struct
{
  long long R; /* Residual capacity of the arc */
  long long U; /* Capacity of the arc */
  long long Tail;
  long long Head;
  long long Mate;
  long long NextInArc;
  long long NextOutArc;
} MXF_ArcRec;
...
typedef struct
{
  long long DLabel;
  long long Excess;
  long long FirstInArc;
  long long LastInArc;
  long long FirstOutArc;
  long long LastOutArc;
  long long CurrentArc;
  long long PrevBNode;    /* Previous Bucket Node */
  long long NextBNode;    /* Next -               */
  long long PrevDLNode;   /* Previous Distance Label Node */
  long long NextDLNode;   /* Next -                       */
} MXF_NodeRec;
...
typedef struct
{
  MXF_ArcPtr ArcPtr;
  MXF_NodePtr NodePtr;
  long long ArcListDim;
  long long ArcListSize;
  long long NodeListDim;
  long long NodeListSize;
  long long *FirstInBPtr;   /* First Node in Bucket[i] */
  long long *FirstInDLPtr;  /* First Node in DistanceLabel[i] */
  long long **ArcMapPtr;
  char *CVWrk1;
  long long *IVWrk1;
  long long *IVWrk2;
...
void MXF_WriteArcList(MaxFlowPtr Ptr)
{
  ...
  printf("ArcList: (ArcListSize=%lld, ArcListDim=%lld)\n",
          P->ArcListSize,P->ArcListDim);
  printf("[NodeList: (NodeListSize=%lld,NodeListDim=%lld]\n",
          P->NodeListSize,P->NodeListDim);
  printf(" Arc#    R    U Tail Head Mate NextOutArc NextInArc\n");
  printf("---------------------------------------------------\n");
  for (i=1; i<=P->ArcListSize; i++)
  printf("%5lld%5lld%5lld%5lld%5lld%5lld%7lld%10lld\n",
         i,
         P->ArcPtr[i].R,
         P->ArcPtr[i].U,
         P->ArcPtr[i].Tail,
...
void MXF_WriteNodeList(MaxFlowPtr Ptr)
{
  ...
  {
    printf("%5lld%4lld%6lld%6lld%6lld%6lld%6lld%10lld\n",
           i,
           P->NodePtr[i].DLabel,
           P->NodePtr[i].Excess,
                         ...
                         long long TotalNodes)
{
  ...
  if (TotalNodes > P->NodeListDim)
  {
    ...
    printf("MXF_SetNodeListSize: TotalNodes > NodeListDim (%lld > %lld)\n",
                 ...
                 long long *Index)
{
  ...
  if ((Tail<=0) || (Tail>P->NodeListSize) ||
      ...
      (Capacity < 0))
  {
    printf("Error in input to MXF_AddArc(NodeListSize=%lld)\n",
           P->NodeListSize);
    exit(0);
  }
                   ...
                   long long Capacity)
{
  ...
  {
    printf("MXF_ChgArcCap: Arc (%lld,%lld) not found => stop.\n",
            Tail,Head);
    exit(0);
  }
}
...
void MXF_ComputeDLabels(MaxFlowPtr Ptr,
                        ...
                        long long Sink)
{
  long long i,n,Index,Tail,Head,Arc;
  long long CurrentLabel,LabeledNodes;
  char *Labeled;
  long long *Node;
  MXF_Ptr P;
...
{
  long long i,k,n;
  long long Arc, ReverseArc;
  long long *InArc, *OutArc, *InArcIndex, *OutArcIndex;
  MXF_Ptr P;
...
void LMXF_Push(MXF_Ptr P,
               long long Arc)
{
  long long Delta,Tail,Head,Mate;
                      ...
                      long long *NodeList)
{
  ...
  char InsertHead;
  long long i,j,n,Node,Tail,Head,Mate,NextInDList,Size,Delta;
  long long Arc;
  long long DLabel,MinLabel,Level,CurrentLevel,ReLabels,ReLabelsLimit;
  MXF_Ptr P;
...
{
  char Shift;
  long long i,j,n,Source,Sink,CutVal,SourceSideSize,SinkNeighbor;
  long long *SourceSide;
  MXF_Ptr P;
  ...
  /*
  printf("CenterNode = %lld\n",CenterNode);
  printf("NextOnPath =");
  for (i=1; i<=n; i++) printf(" %lld",NextOnPath[i]);
  printf("\n");
  printf("CutValue =");
  for (i=1; i<=n; i++) printf(" %lld",CutValue[i]);
  */
}
